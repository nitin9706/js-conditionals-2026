...
export function calculateShipping(weight, country, orderTotal) {
  ...
  if (country === "US") {
    if (weight <= 1 && orderTotal <= 50) return 5;

    if (weight <= 5 && orderTotal <= 50) return 10;

    if (weight > 5 && orderTotal <= 50) return 15;

    if (orderTotal > 50) return 0;
  }
  if (country !== "US") {
    if (weight <= 1 && orderTotal <= 100) return 15;

    if (weight <= 5 && orderTotal <= 100) return 25;

    if (weight > 5 && orderTotal <= 100) return 40;
